...
function handle_inputs() {
	...
	let _space_type;
	if (!["static", "khulnasoft"].includes(space_type)) {
		core.setFailed(
			`'${space_type}' is not a supported space type. Only 'khulnasoft' and 'static' are supported.`
		);
		throw new Error();
	} else {
		_space_type = space_type as "khulnasoft" | "static";
	}
...
async function create_comment(
	...
	const comment_tag_pattern = `<!-- khulnasoft/deploy-space -->`;
